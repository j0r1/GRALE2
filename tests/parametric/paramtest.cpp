...
int main(void)
{
...
#if 1
	vector<pair<size_t,string>> originParams = { { 0, "{ return x*2; }"}, { 1, "{ return x*0.5;}" }};
	vector<float> corrFactors = { 2.0, 0.5 };
	size_t numOriginParams = 2;
#else
	vector<pair<size_t,string>> originParams;
	vector<float> corrFactors = { 1.0, 1.0 };
	...
	for (auto &changedParams : allChangedParams)
	{
		...
		size_t numGenomes = changedParams.size()/changeableParamIdx.size();
		for (size_t i = 0 ; i < numGenomes ; i++)
		{
			vector<float> floatParamsMod = floatParams;
			for (size_t j = 0 ; j < changeableParamIdx.size() ; j++)
				floatParamsMod.at(changeableParamIdx[j]) = changedParams.at(i*changeableParamIdx.size() + j) * corrFactors.at(j);
