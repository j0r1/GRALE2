									...
									double *pTarget, char *errMsgs)
{
	...
	nodeResults.resize(numPixels * 5);
	...
	for (int i = 0, off = 0, j = offsets[rank] ; i < numPixels ; i++, off += 5, j++)
	{
		...
		nodeResults[off+0] = alpha.getX();
		nodeResults[off+1] = alpha.getY();
		nodeResults[off+2] = axx;
		nodeResults[off+3] = ayy;
		nodeResults[off+4] = axy;
		...
		double *pTarget, char *errMsgs)
{
	...
	// Offsets are just pixel offsets, but each node creates 5 doubles per pixel
	for (size_t i = 0 ; i < doubleOffsets.size() ; i++)
		doubleOffsets[i] = (offsets[i]/2) * 5; // dividing by two again because offsets had become number of doubles
