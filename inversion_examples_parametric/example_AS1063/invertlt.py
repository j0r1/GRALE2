    ...
    "params": [
        {
            ...
            "lens": {
                "type": "LTPIEMDLens",
                "params": {
                    "velocitydispersion": { "initmin": 1000000, "initmax": 1500000 },
                    ...
                    "ellipticity": { "initmin": 0.1, "initmax": 0.9 }
                }
            }
        }
    ]
...
def initialInversion(imgList, paramDesc):
    ...
    # Invert!
    r = iws.invertParametric(paramDesc, 512, eaType = "GA",
                             useImagePositionRandomization = True)
    startLens = r["lens"]
    startLens.save("startInv2.lensdata")
    ...
    # explore parameter uncertainties. We'll also return the initial
    # scale factors, so that they can be used in the following inversion -
    # this can help comparing raw sample data between inversion.
    return paramdesc.refineParametricDescription(paramDesc, startLens, 0.001), r["scales"]
...
def mcmcInversion(imgList, paramDesc, scales):
    ...
    mcmcParams = { "burningenerations": burnInGenerations, "samplesfilename": "samples2.dat" }
    ...
    r = iws.invertParametric(paramDesc, 512, maximumGenerations = totalGenerations, eaType = "MCMC", 
                             geneticAlgorithmParameters=mcmcParams, fitnessObjectParameters=fitParams,
                             forceScales = scales)

    lens = r["lens"]
    lens.save("mcmcInv2.lensdata")
    ...
    pickle.dump(r["optparams"], open("paraminfo2.dat", "wb"))

# Run the initial inversion to get a good starting point for the MCMC part
newParamDesc, scales = initialInversion(imgList, initialParamDesc)
# Do the actual MCMC inversion
mcmcInversion(imgList, newParamDesc, scales)
